...
$sql = "SELECT hoa_don_id, hoa_don.khach_hang_id, khach_hang.ho_ten_kh AS ten_KH, nhan_vien.ho_ten AS TenNV, mon_an.ten_mon_an AS ten_mon, so_luong, so_luong*gia_tien AS tong_tien
FROM khach_hang
INNER JOIN hoa_don ON khach_hang.khach_hang_id = hoa_don.khach_hang_id
INNER JOIN nhan_vien ON hoa_don.nhan_vien_id = nhan_vien.nhan_vien_id
INNER JOIN mon_an ON hoa_don.ten_mon_an = mon_an.ten_mon_an";

$result = $conn->query($sql); 

$conn->close(); 
?>    
<table border="1" width="100%" style="text-align:center; border-color:black">
<tr>
<th>Hóa đơn ID</th>
<th>Khách hàng ID</th>
<th>Ho và tên</th>
...
{
if($row["hoa_don_id"]%2 == 0)
{
echo"<tr class='odd'><td>".$row['hoa_don_id']."</td>";
}
else 
{
echo "<tr class='even'><td>".$row['hoa_don_id']."</td>";

}
echo"<td>".$row['hoa_don_id']."</td>". 
   "<td>".$row['ten_KH']."</td>".
   "<td>".$row['TenNV']."</td>".
   "<td>".$row['ten_mon']."</td>".
   "<td>".$row['so_luong']."</td>".
   "<td>".$row['tong_tien']."</td>".
   "<td>"."<form action='deleteHD.php' method='get'><input type='submit' value='Xóa' name='id'></form>
   "."</td>
   </tr>";
}
?>
